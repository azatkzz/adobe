...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-black/40 backdrop-blur-xl border-t border-white/10">
      <div className="flex justify-around items-center h-full max-w-lg mx-auto">
        {navItems.map((item) => (
          <button 
            key={item.path}
            className="p-2 group relative flex flex-col items-center"
